...
class JobStore:
    ...
    def create_job(self, audio_bytes: bytes, extension: str, params: dict) -> str:
        ...
        job_dir = self._job_dir(job_id)
        os.makedirs(job_dir)
        with open(os.path.join(job_dir, f"audio{extension}"), "wb") as handle:
    ...
    def params(self, job_id: str) -> dict:
        job_dir = self._require_job_dir(job_id)
        with open(os.path.join(job_dir, "params.json"), encoding="utf-8") as handle:
            return json.load(handle)

    def get_status(self, job_id: str) -> dict:
        job_dir = self._require_job_dir(job_id)
        with open(os.path.join(job_dir, "status.json"), encoding="utf-8") as handle:
            return json.load(handle)
    ...
    def delete_job(self, job_id: str) -> None:
        shutil.rmtree(self._require_job_dir(job_id))
